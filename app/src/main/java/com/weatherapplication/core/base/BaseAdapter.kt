...
abstract class BaseAdapter<T : Item, VB : ViewBinding>(diffCallback: ViewBindingDiffUtilCallback<T>) :
    ListAdapter<T, BaseAdapter.ViewBindingViewHolder<T, VB>>(diffCallback) {
    override fun onBindViewHolder(holder: ViewBindingViewHolder<T, VB>, position: Int) = holder.bind(item = getItem(position))

    override fun onBindViewHolder(holder: ViewBindingViewHolder<T, VB>, position: Int, payloads: MutableList<Any>) {
